...
<div className="flex items-center mb-2">
                        <div className="flex text-orange-400 text-sm">
                            {[...Array(5)].map((_, i) => {
                                const avgRating = product.reviews_avg_rating ? parseFloat(product.reviews_avg_rating) : 0;
                                const isFilled = i < Math.floor(avgRating);
                                const isHalfFilled = i === Math.floor(avgRating) && avgRating % 1 >= 0.5;
                                return (
                                    <svg
                                        key={i}
                                        className={`w-4 h-4 ${isFilled || isHalfFilled ? 'fill-current' : 'stroke-current'}`}
